...
// CSV import endpoint: supports ?strategy=replace|upsert|append
router.post('/assets/import', upload.single('file'), async (req,res)=>{
  ...
  try {
    ...
    for await (const row of parser) {
      // adjust CSV column names to match your file
      await client.query(
        ...
        [row.asset_no, row.name, row.model, row.serial_number, row.purchase_date || null, row.location, row.owner, row.department, row.price || null]
      );
    }
